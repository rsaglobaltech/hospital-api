...
class GetMedicalStaffBySpecialtyQueryHandlerTest {

    @Mock
    private MedicalStaffRepository medicalStaffRepository;

    private GetMedicalStaffBySpecialtyQueryHandler handler;

    @BeforeEach
    void setUp() {
        MockitoAnnotations.openMocks(this);
        handler = new GetMedicalStaffBySpecialtyQueryHandler(medicalStaffRepository);
    }

    @Test
    void testHandleGetMedicalStaffBySpecialtyQuery() {
        // Arrange
        Specialty specialty = Specialty.CARDIOLOGY;
        
        // Create first medical staff with CARDIOLOGY specialty
        ...
        LocalDate dateOfBirth1 = LocalDate.of(1980, 1, 1);
        String address1 = "123 Main St, City, Country";
        LocalDate hireDate1 = LocalDate.of(2010, 1, 15);
        ...
        when(medicalStaff1.getDateOfBirth()).thenReturn(new DateOfBirth(dateOfBirth1));
        when(medicalStaff1.getAddress()).thenReturn(new Address(address1));
        when(medicalStaff1.getSpecialty()).thenReturn(specialty);
        when(medicalStaff1.getQualifications()).thenReturn(qualifications1);
        when(medicalStaff1.isActive()).thenReturn(true);
        when(medicalStaff1.getHireDate()).thenReturn(new HireDate(hireDate1));
        when(medicalStaff1.getTerminationDate()).thenReturn(null);
        
        // Create second medical staff with CARDIOLOGY specialty
        ...
        LocalDate dateOfBirth2 = LocalDate.of(1985, 5, 10);
        String address2 = "456 Oak St, Town, Country";
        LocalDate hireDate2 = LocalDate.of(2015, 3, 20);
        ...
        when(medicalStaff2.getDateOfBirth()).thenReturn(new DateOfBirth(dateOfBirth2));
        when(medicalStaff2.getAddress()).thenReturn(new Address(address2));
        when(medicalStaff2.getSpecialty()).thenReturn(specialty);
        when(medicalStaff2.getQualifications()).thenReturn(qualifications2);
        when(medicalStaff2.isActive()).thenReturn(true);
        when(medicalStaff2.getHireDate()).thenReturn(new HireDate(hireDate2));
        ...
        when(medicalStaffRepository.findBySpecialty(specialty)).thenReturn(medicalStaffList);
        ...
        assertEquals(specialty, response1.getSpecialty());
        ...
        assertEquals(specialty, response2.getSpecialty());
    ...
    @Test
    void testHandleGetMedicalStaffBySpecialtyQueryWithEmptyList() {
        // Arrange
        Specialty specialty = Specialty.CARDIOLOGY;
        when(medicalStaffRepository.findBySpecialty(specialty)).thenReturn(new ArrayList<>());
