...
/**
 * Handler for retrieving prescriptions by patient ID
 */
@Service
@RequiredArgsConstructor
public class GetPrescriptionsByPatientIdQueryHandler implements QueryHandler<GetPrescriptionsByPatientIdQuery, List<PrescriptionResponse>> {
    ...
    public List<PrescriptionResponse> handle(GetPrescriptionsByPatientIdQuery query) {
        List<Prescription> prescriptions;

        if (query.isOnlyValid()) {
            prescriptions = prescriptionRepository.findValidByPatientId(query.getPatientId());
        } else {
            prescriptions = prescriptionRepository.findByPatientId(query.getPatientId());
        }
